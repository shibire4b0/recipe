(() => {
  ...
  /* ============ storage ============ */
  const FOODS_KEY = 'recipeapp_foods';
  const RECIPES_KEY = 'recipeapp_recipes';

  const NUMERIC_UNITS = ['ミリリットル', 'CC', 'グラム'];
  const LIST_UNITS = ['個', '大さじ', '小さじ', 'カップ', '合', 'その他'];
  ...
  // temp state used only while inside the food-select -> unit sub-flow
  let pendingFoodIds = [];
  ...
  function renderMenu() {
    ...
    app.innerHTML = `
      <div class="screen">
        ...
        <div class="screen-footer">
          <button class="btn btn-dark" data-nav="#/foods/new">食材登録</button>
          <button class="btn btn-dark" data-nav="#/add-recipe">レシピ作成</button>
        </div>
      </div>
    `;
    ...
    function draw() {
      ...
      if (filtered.length === 0) {
        listEl.innerHTML = `<div class="empty-msg">${recipes.length === 0 ? 'まだレシピがありません。「レシピ作成」から追加しましょう。' : '該当するレシピが見つかりません。'}</div>`;
        return;
      }
  ...
  function renderRecipeDetail(id) {
    ...
    app.innerHTML = `
      <div class="screen">
        ${recipe.memo ? `<div class="memo-bar"><b>memo</b>${escapeHtml(recipe.memo)}</div>` : ''}
        <div class="screen-body" id="detailBody">
          <div class="detail-head">
            <h1 class="detail-title">${escapeHtml(recipe.name || '無題のレシピ')}</h1>
            <button class="edit-btn" data-nav-edit="${recipe.id}">編集</button>
          </div>
          ${recipe.image ? `<img src="${recipe.image}" alt="" style="width:100%;border-radius:18px;border:3px solid var(--green);box-shadow:6px 6px 0 var(--green);margin:10px 0 18px;max-height:260px;object-fit:cover;">` : ''}
          <div class="card">
            <h2 style="font-family:var(--font-display);margin:0 0 8px;">INGREDIENTS</h2>
            ${ingredientsHtml}
          </div>
          <div class="card">
            <h2 style="font-family:var(--font-display);margin:0 0 8px;">COOKING</h2>
            ${stepsHtml}
          </div>
  ...
  function renderAddRecipe() {
    ...
    app.innerHTML = `
      <div class="screen grid-bg">
        <div class="screen-topbar"></div>
        <div class="screen-header">
          <h1 class="screen-title">ADD RECIPE</h1>
          <div class="screen-subtitle">INGREDIENTS SELECT</div>
        </div>
        <div class="screen-body">
          ...
          <div class="field-label">メモを追加</div>
          <textarea class="textarea-input" id="memoInput" placeholder="（任意）保存のコツなどメモを残せます">${escapeHtml(draft.memo)}</textarea>
        </div>
        <div class="screen-footer">
          <button class="btn btn-outline" id="cancelBtn">取り消し</button>
          <button class="btn btn-dark" id="nextBtn">材料登録</button>
        </div>
      </div>
    `;
  ...
  function renderAddFoodSelect() {
    ...
    app.innerHTML = `
      <div class="screen grid-bg">
        <div class="screen-topbar"></div>
        <div class="screen-header">
          <h1 class="screen-title">ADD FOOD</h1>
          <div class="screen-subtitle">INGREDIENTS SELECT</div>
        ...
        <div class="screen-footer">
          <button class="btn btn-outline" id="cancelBtn">取り消し</button>
          <button class="btn btn-dark" id="confirmBtn">材料登録</button>
        </div>
      </div>
    `;
    ...
    function draw() {
      ...
      if (list.length === 0) {
        gridEl.innerHTML = `<div class="empty-msg" style="grid-column:1/-1;">食材が見つかりません。メニュー画面の「食材登録」から追加してください。</div>`;
        return;
      }
  ...
  function renderAddUnit() {
    ...
    app.innerHTML = `
      <div class="screen grid-bg">
        <div class="screen-topbar"></div>
        <div class="screen-header">
          <h1 class="screen-title">ADD UNIT</h1>
          <div class="screen-subtitle">INGREDIENTS SELECT</div>
        </div>
  ...
  function renderAddStep() {
    ...
    app.innerHTML = `
      <div class="screen grid-bg">
        <div class="screen-topbar"></div>
        <div class="screen-header">
          <h1 class="screen-title">ADD STEP</h1>
  ...
  function renderFinish() {
    ...
    app.innerHTML = `
      <div class="screen">
        ${draft.memo ? `<div class="memo-bar"><b>memo</b>${escapeHtml(draft.memo)}</div>` : ''}
        <div class="screen-body">
          ${draft.image ? `<img src="${draft.image}" alt="" style="width:100%;border-radius:18px;border:3px solid var(--green);box-shadow:6px 6px 0 var(--green);margin:14px 0 4px;max-height:220px;object-fit:cover;">` : ''}
          <div class="card">
            <h2 style="font-family:var(--font-display);margin:0 0 8px;">INGREDIENTS</h2>
            ${ingredientsHtml}
          </div>
  ...
  function renderFoodNew() {
    ...
    app.innerHTML = `
      <div class="screen grid-bg">
        <div class="screen-topbar"></div>
        <div class="screen-header">
          <h1 class="screen-title">INGREDIENTS</h1>
        </div>
        <div class="screen-body">
          <input id="foodNameInput" class="text-input" type="text" placeholder="ここに登録する材料名を入力" style="margin-bottom:18px;">
          <div class="screen-subtitle">UNIT SELECT</div>
